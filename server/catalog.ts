...
const usesLocalTelegramApi = telegramApiBase !== "https://api.telegram.org";
const defaultUploadLimitBytes = process.env.TELEGRAM_BOT_TOKEN
  ? usesLocalTelegramApi
    ? 2 * 1024 * 1024 * 1024
    : 50 * 1024 * 1024
  : 5 * 1024 * 1024 * 1024;
const uploadLimitBytes = Number(
  process.env.MAX_UPLOAD_BYTES || defaultUploadLimitBytes,
);

function formatUploadLimit(bytes: number) {
  if (bytes >= 1024 ** 3) return `${Math.floor(bytes / 1024 ** 3)} GB`;
  return `${Math.floor(bytes / 1024 ** 2)} MB`;
}
...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename: (_req, file, callback) =>
      callback(
        null,
        `${Date.now()}-${crypto.randomUUID()}-${file.originalname.replace(/[^a-zA-Z0-9._-]/g, "-")}`,
      ),
  }),
...
function uploadMedia(req: Request, res: Response, next: () => void) {
  upload.single("file")(req, res, (error: any) => {
    if (error?.code === "LIMIT_FILE_SIZE") {
      res.status(413).json({
        error: `File is too large. Maximum configured size is ${formatUploadLimit(uploadLimitBytes)}.`,
      });
      return;
    }
    if (error) {
...
export function registerCatalogRoutes(app: Express) {
      ...
      res.json(
        items.map((item) => ({
          ...
          thumbnailUrl: item.thumbnailFileId
            ? `/manus-storage/telegram/${item.thumbnailFileId}`
            : undefined,
  ...
  app.get("/api/catalog/upload-limit", (_req: Request, res: Response) => {
    res.json({
      ...
      maxGigabytes: uploadLimitBytes / 1024 ** 3,
      mode: "disk-stream",
    });
  });
      ...
      const allowed = {
        ...
        artist:
          typeof req.body.artist === "string"
            ? req.body.artist.trim().slice(0, 255)
            : undefined,
        published:
          typeof req.body.published === "boolean" ||
          req.body.published === 0 ||
          req.body.published === 1
            ? req.body.published === true || req.body.published === 1
              ? 1
              : 0
            : undefined,
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        try {
          ...
          const telegram = telegramConfigured()
            ? await telegramPut(
                ...
                `${req.body.title || req.file.originalname} | ${req.body.artist || "Mg Flâsh"}`,
              )
            : null;
          const uploaded = telegram
            ? {
                key: `telegram/${telegram.fileId}`,
                url: `/manus-storage/telegram/${telegram.fileId}`,
              }
            : await storagePut(
                key,
        ...
        res
          .status(
            message.includes("Forbidden") || message.includes("Owner")
              ? 403
              : 500,
          )
          .json({ error: message });
