...
export default function HomeScreen() {
  ...
  return (
    <ScreenContainer className="px-5 pt-3" edges={["top", "left", "right"]}>
      ...
      <View className="mt-6 flex-row items-center justify-between"><View><Text className="text-lg font-bold text-foreground">{category === "video" ? "Video" : category === "audio" ? "Music" : "All media"}</Text><Text className="mt-1 text-sm text-muted">{filtered.length} {filtered.length === 1 ? "file" : "files"} · tap to play</Text></View><View className="flex-row items-center rounded-full border border-primary/40 px-3 py-2"><MaterialIcons name="cloud" size={16} color="#69c7e8" /><Text className="ml-1 text-xs font-semibold text-primary">Telegram + Local</Text></View></View>

      {!hydrated ? <View className="flex-1 items-center justify-center"><ActivityIndicator color="#69c7e8" /></View> : filtered.length ? <View className="mt-5 flex-1">{filtered.slice(0, 10).map((item) => <MediaCard key={item.id} item={item} onPress={() => { playFromList(item.id, filtered.map((entry) => entry.id)); router.push(`/player/${item.id}`); }} onFavorite={() => void toggleFavorite(item.id)} onDownload={() => void downloadMedia(item.id)} />)}{filtered.length > 10 ? <Text className="mt-2 text-center text-xs text-muted">Showing the top 10 closest matches</Text> : null}</View> : <View className="mt-10 items-center rounded-3xl border border-dashed border-border bg-surface/70 px-7 py-10"><View className="h-16 w-16 items-center justify-center rounded-2xl bg-primary/15"><MaterialIcons name={category === "video" ? "movie" : category === "audio" ? "music-note" : "cloud-upload"} size={32} color="#69c7e8" /></View><Text className="mt-5 text-center text-xl font-bold text-foreground">{query ? "No close matches yet" : "No media yet"}</Text><Text className="mt-2 text-center leading-6 text-muted">{query ? "Try fewer words or a different artist/title." : "Import a local Video/Music file in Settings, or refresh for Telegram uploads."}</Text></View>}
      </ScrollView>
      <MiniPlayer />
    </ScreenContainer>
  );
