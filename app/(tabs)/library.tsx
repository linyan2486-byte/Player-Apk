...
export default function LibraryScreen() {
  const { library, playFromList, deleteMedia, toggleFavorite, downloadMedia } =
    usePlayer();
  const [query, setQuery] = useState("");
  const [filter, setFilter] = useState<"all" | "audio" | "video" | "favorite">(
    "all",
  );

  const downloaded = useMemo(
    () => library.filter((item) => Boolean(item.remoteId && item.offline)),
    [library],
  );
  const filtered = useMemo(() => {
    const byFilter = downloaded.filter(
      (item) =>
        filter === "all" ||
  ...
  return (
    <ScreenContainer className="px-5 pt-3" edges={["top", "left", "right"]}>
      <Text className="text-3xl font-bold text-foreground">Offline</Text>
      <Text className="mt-1 text-sm text-muted">
        Downloaded Telegram Video and Music for offline playback.
      </Text>
      ...
      <View className="mt-4 flex-row gap-2">
        {(["all", "video", "audio", "favorite"] as const).map((option) => (
          ...
          >
            <View
              className={`flex-row items-center rounded-full px-3 py-2 ${filter === option ? "bg-primary" : "border border-border bg-surface"}`}
            >
              <MaterialIcons
                name={
                  option === "video"
                ...
                size={15}
                color={filter === option ? "#07131a" : "#7f8c9a"}
              />
              <Text
          ...
          filtered.map((item) => (
            <View key={item.id}>
              <MediaCard
                ...
                onDelete={() => askDelete(item.id, item.title)}
              />
            </View>
          ))
        ) : (
          <View className="mt-16 items-center">
            <MaterialIcons name="download" size={42} color="#50606e" />
            <Text className="mt-3 text-base text-muted">
              {query ? "No close matches found." : "No downloaded files yet."}
            </Text>
            <Text className="mt-2 text-center text-sm text-muted">
              Download a Video or Music item from Home to use it offline.
            </Text>
          </View>
        )}
      </View>
